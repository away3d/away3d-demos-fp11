package utils
{
    ...
    public class BitmapFilterEffects extends Sprite
	{
		...
		static public function normalMap(sourceBitmap:BitmapData, amount:Number = 10, soft_sobel:Number = 1, invert_red:Number = -1, invert_green:Number = -1):BitmapData
		{
			...
			_normalMapShader.data.soft_sobel.value = [soft_sobel]; //int 0 or 1
			_normalMapShader.data.invert_red.value = [invert_red]; //-1 to 1
			_normalMapShader.data.invert_green.value = [invert_green]; //-1 to 1
		...
		static public function outline(sourceBitmap:BitmapData, difference:Array, color:uint = 0xFFFFFF, bgcolor:uint = 0x000000):BitmapData
		{
			var returnBitmap:BitmapData = new BitmapData(sourceBitmap.width, sourceBitmap.height, sourceBitmap.transparent, 0x0);
			var data:ShaderData = _outlineShader.data;
			
			
			_outlineShader.data.difference.value = [1, 0.15];
			_outlineShader.data.color.value = [((color & 0xFF0000) >> 16) / 255, ((color & 0x00FF00) >> 8) / 255, (color & 0x0000FF) / 255, 1];
			_outlineShader.data.bgcolor.value = [((bgcolor & 0xFF0000) >> 16) / 255, ((bgcolor & 0x00FF00) >> 8) / 255, (bgcolor & 0x0000FF) / 255, 1];
