package utils
{
	...
	public class AvatarMapper extends Sprite
	{
		...
		private static var style:Object;
		
		public function AvatarMapper()
		{
		...
		{
			style = {uni: true, nakedLeg: true, nakedNeck: false, nakedFoot: true}
			...
			//-----------------------------tint line layer
			s.graphics.clear();
			s.graphics.beginFill(cSkin); // skin
			...
			s.graphics.beginFill(cShirt); // shirt
			s.graphics.drawRect(0, 180, 512, 144);
			// neckstyle
			s.graphics.beginFill(cTrousor); // trousor
			s.graphics.drawRect(0, 324, 512, 148);
			if (style.nakedFoot)
				s.graphics.beginFill(cSkin);
			else
				s.graphics.beginFill(0x1A130F); // shoes
			s.graphics.drawRect(0, 472, 512, 40);
			s.graphics.endFill();
			if (style.nakedLeg)
			{
				s.graphics.beginFill(cSkin); //extra woman leg
			...
			//-----------------------------deco line layer
			s.graphics.clear();
			s.graphics.beginFill(0x121212); // eye
			...
			s.graphics.endFill();
			s.graphics.beginFill(0x603310); // eye color
			s.graphics.drawEllipse(223, 77, 8, 8);
			s.graphics.drawEllipse(282, 77, 8, 8);
			s.graphics.endFill();
			s.graphics.beginFill(0x121212);
			s.graphics.drawEllipse(225, 79, 4, 4); // eye
			s.graphics.drawEllipse(284, 79, 4, 4); // eye
			s.graphics.endFill();
			s.graphics.beginFill(0x121212, 0.3); // mouth
