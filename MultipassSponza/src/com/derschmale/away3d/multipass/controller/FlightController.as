package com.derschmale.away3d.multipass.controller {
	...
	public class FlightController
	{
		...
		{
			// todo: take cam properties and reset current data
			_stage.addEventListener(MouseEvent.MOUSE_DOWN, onMouseDown);
			_stage.addEventListener(MouseEvent.MOUSE_UP, onMouseUp);
			_stage.addEventListener(KeyboardEvent.KEY_DOWN, onKeyDown);
			_stage.addEventListener(KeyboardEvent.KEY_UP, onKeyUp);
			if (_autoUpdate) _stage.addEventListener(Event.ENTER_FRAME, onEnterFrame);
		}
		...
		public function stop():void
		{
			...
			if (_autoUpdate) _stage.removeEventListener(Event.ENTER_FRAME, onEnterFrame);
			_drag = false;
		}
